...
describe Role::RequestService do
  ...
  let(:result_file_headers) { { "content-disposition" => "attachment; filename=#{result_file_name}" } }
  ...
  it "should raise ResultTokenError exception" do
    stub_request(:post, url).with(body: "query=#{query}").to_return(status: 200, body: request_token_fixture)
  ...
  it "should raise EmptyResultRequestError exception" do
    stub_request(:post, url).with(body: "query=#{query}").to_return(status: 200, body: request_token_fixture)
    stub_request(:get, "#{url}/search-result/#{request_token}").to_return(status: 200, body: empty_result_token_fixture)
  ...
  it "should raise ResultRequestError exception" do
    stub_request(:post, url).with(body: "query=#{query}").to_return(status: 200, body: request_token_fixture)
    stub_request(:get, "#{url}/search-result/#{request_token}").to_return(status: 200, body: result_token_fixture)
  ...
  it "should raise ResultStatusTimeoutError exception" do
    stub_request(:post, url).with(body: "query=#{query}").to_return(status: 200, body: request_token_fixture)
    stub_request(:get, "#{url}/search-result/#{request_token}").to_return(status: 200, body: result_token_fixture)
    stub_request(:get, "#{url}/vyp-request/#{result_token}").to_return(status: 200, body: result_request_fixture)
    stub_request(:get, "#{url}/vyp-status/#{result_token}").to_return(status: 200, body: result_status_waiting_fixture)
  ...
  it "should raise ResultFileError exception" do
    ...
    stub_request(:get, "#{url}/search-result/#{request_token}").to_return(status: 200, body: result_token_fixture)
    stub_request(:get, "#{url}/vyp-request/#{result_token}").to_return(status: 200, body: result_request_fixture)
    stub_request(:get, "#{url}/vyp-status/#{result_token}").to_return(status: 200, body: result_status_ready_fixture)
  ...
  it "should not raise any errors" do
    ...
    stub_request(:get, "#{url}/vyp-request/#{result_token}").to_return(status: 200, body: result_request_fixture)
    stub_request(:get, "#{url}/vyp-status/#{result_token}").to_return(status: 200, body: result_status_ready_fixture)
    stub_request(:get, "#{url}/vyp-download/#{result_token}").to_return(status: 200, body: result_file_fixture, headers: result_file_headers)
